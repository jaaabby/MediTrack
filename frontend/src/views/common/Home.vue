<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-blue-100 mt-1">
            {{ authStore.isDoctor ? 'Panel de Solicitudes Médicas' : authStore.isPavedad ? 'Panel de Visualización de Solicitudes' : authStore.isPavilionUser ? 'Panel de Gestión de Pabellón' : authStore.isNurse ? 'Panel de Gestión de Inventario' : 'Sistema de gestión de inventario médico' }}
          </p>
        </div>
    ...
    <!-- Barra de búsqueda principal - Solo para roles que pueden ver inventario (excepto pabellón) -->
    <div v-if="!authStore.isDoctor && !authStore.isPavedad && !authStore.isNurse && !authStore.isPavilionUser" class="card">
      ...
      <div class="flex items-end gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 md:flex-shrink-0">
          ...
          <button @click="searchQuery = ''" :disabled="!searchQuery" class="btn-secondary w-full sm:w-auto flex items-center justify-center whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed">
            Limpiar filtros
          </button>
        </div>
      </div>
    ...
    <div v-if="authStore.isDoctor" class="max-w-4xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <!-- Escanear QR - Retirar y Recepcionar -->
        <div class="card hover:shadow-lg transition-all duration-200 cursor-pointer transform hover:scale-105" @click="navigateTo('/qr')">
          <div class="text-center p-6">
            ...
            <h3 class="text-xl font-semibold text-gray-900 mb-2">Escanear QR</h3>
            <p class="text-gray-600">Retirar insumos de bodega y recepcionarlos en el pabellón</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Consignación ahora usa la misma vista que encargado de bodega -->
